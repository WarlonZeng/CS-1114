...
def printStudentDonationData( dictyKeys ):
    ...
    print('')
    for index in range(len(dictyKeys)+1): # for some reason dictyKeys gives me a dictionary and a list.. if you do type(dictyKeys)
        print(dicty.items()[index][0], '$' + str(dicty.items()[index][1]))

    
def sortAlphabetically(dicty):
    'sorts alphabetically; dictykeys is list and dictionary...'
    dictyKeys = sorted(dicty, key=dicty.get)
    return dictyKeys
...
def donationProgram(keyVal, dicty):
    ...
    return dicty, dictyKeys


def main():
    'the program'
    dictyKeys = donationProgram(keyVal, dicty)
